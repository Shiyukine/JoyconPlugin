//Made by mfosse for vJoy
//Edited by linkoid for FreePIE (Edits are marked with //linkoid)
...
//For a hack to move data from a .exe to .dll because I know nothing about C
...
//linkoid
namespace JoyconDataSpace {
	/*struct JoyconData {
		/// JOYSTICK_POSITION
		int 	bDevice;	 // Index of device. 1-based.

		///Simple Joycon Inputs
		int lx;
		int ly;
		int lc;
	...
	/*void to_json(json& j, const JoyconData& x) {
		j = json{
			{ "bDevice",   x.bDevice },
...
JoyconDataSpace::JoyconData iReport; // The structure that holds the full position data

//linkoid - create shared memory
//#include <windows.h>
#include <stdio.h>
#include <conio.h>
#include <tchar.h>
HANDLE JoyconMapFile;
LPVOID JoyconMapBuffer;

int CopyToBuffer(LPCTSTR pBuff, TCHAR szMsg[])
{
	//printf("%i - %i = %i", BUF_SIZE, _tcslen(szMsg) * sizeof(TCHAR), BUF_SIZE - _tcslen(szMsg) * sizeof(TCHAR));
	TCHAR underflowBuffer[] = TEXT("                                                                                                                                                                                                                                                               ");
	//_tcscat_s(szMsg, _tcslen(szMsg)+_tcslen(underflowBuffer)+ BUF_SIZE, underflowBuffer);
	CopyMemory((PVOID)pBuff, szMsg, _tcslen(szMsg) * sizeof(TCHAR));
	char printBuf[BUF_SIZE];
	strcpy_s(printBuf, BUF_SIZE, CT2CA(pBuff));
	printf("%s\n", printBuf);
	return 0;
}
...
//linkoid - so we can track each controiller seperately
std::array<Tracker, 16> trackers;
...
//void updatevJoyDevice2(Joycon *jc, int trueI) {
//
//	UINT DevID;
//
//	PVOID pPositionMessage;
...
//
//	// gyro / accelerometer data:
//	if (true) { //((jc->left_right == a) || (joycons.size() == 1 && jc->left_right == b) || (jc->left_right == 3)) {
//		Tracker tracker = trackers[trueI];
//		int multiplier;
//
//
//		// Gyroscope (roll, pitch, yaw):
...
//		//}
//
//		float mult = 1;//settings.gyroSensitivityX / 100.0f;
//
//					   /*iReport.wAxisZRot = 16384 + (jc->gyro.roll * mult);
//					   iReport.wSlider = 16384 + (jc->gyro.pitch * mult);
//					   iReport.wDial = 16384 + (jc->gyro.yaw * mult);*/
//
//					   //linkoid
//		iReport.mr = (jc->gyro.roll);
//		iReport.mp = (jc->gyro.pitch);
//		iReport.my = (jc->gyro.yaw);
//
//		iReport.gr = roll;
//		iReport.gp = pitch;
//		iReport.gy = yaw;
...
//
//	if (jc->left_right == 1) {
//		iReport.down = jc->btns.down;
//		iReport.up = jc->btns.up;
//		iReport.right = jc->btns.right;
//		iReport.left = jc->btns.left;
//		iReport.sr = jc->btns.sr;
//		iReport.sl = jc->btns.sl;
//		iReport.l = jc->btns.l;
//		iReport.zl = jc->btns.zl;
//		iReport.minus = jc->btns.minus;
//		iReport.lc = jc->btns.stick_button;
//		iReport.capture = jc->btns.capture;
//	}
//
//	if (jc->left_right == 2) {
//		iReport.y = jc->btns.y;
...
//		iReport.sl = jc->btns.sl;
//		iReport.r = jc->btns.r;
//		iReport.zr = jc->btns.zr;
//		iReport.plus = jc->btns.plus;
//		iReport.rc = jc->btns.stick_button;
//		iReport.home = jc->btns.home;
//	}
//
//	iReport.ax = jc->accel.x;
//	iReport.ay = jc->accel.y;
//	iReport.az = jc->accel.z;
//
//	//iReport.lButtons = btns;
//
//	// Send data to vJoy device
...
//	//linkoid #HACK
//	json j(iReport);
//	TCHAR jsonTCHAR[BUF_SIZE];
//	//printf((j.dump()+"\n").c_str());
//	_tcscpy_s(jsonTCHAR, CA2T(j.dump().c_str()));
//	CopyToBuffer(JoyconMapBuffer, jsonTCHAR);
//	//System::FileStream fileStream = System.IO::FileStream::gcnew FileStream(name, FileMode::Open, FileAccess::Read, FileShare::Read);
//	//filewriter.open("plugins\\JoyconPlugin\\Joycon.json");
//	//filewriter << j.dump();
//	//filewriter.close();
//
//
...
void updatevJoyDevice2(Joycon *jc, int trueI) {
	...
	iReport.buttons = jc->buttons;
	iReport.type = jc->left_right;

	//iReport.buttons = btns;

	// Send data to vJoy device
	unsigned char num = (unsigned char)(((char*)JoyconMapBuffer)[0]);
	num = (unsigned char)(num + 1);
	char pMessage[BUF_SIZE];
	pMessage[0] = num;
	for (int i = 1; i <= sizeof(iReport); i++) {
		pMessage[i] = ((char*)&iReport)[i-1];
	}
	for (int i = sizeof(iReport)+1; i < BUF_SIZE; i++) {
		pMessage[i] = ((char*)JoyconMapBuffer)[i-sizeof(iReport)];
	}

	CopyMemory(JoyconMapBuffer, (LPVOID)pMessage, BUF_SIZE);
...
void parseSettings2() {
	...
	//linkoid : I changed the config file location
	std::map<std::string, std::string> cfg = LoadConfig("plugins\\JoyconPlugin\\config.txt");
...
void start() {
	...
	//linkoid
	JoyconMapFile = CreateFileMapping(
		INVALID_HANDLE_VALUE,           // use paging file
		NULL,                           // default security
		PAGE_READWRITE,                 // read/write access
		0,                              // maximum object size (high-order DWORD)
		BUF_SIZE,                       // maximum object size (low-order DWORD)
		TEXT("Global\\JoyconMapFile")); // name of mapping object


	if (JoyconMapFile == NULL)
	{	
		JoyconMapFile = OpenFileMapping(FILE_MAP_ALL_ACCESS, false, TEXT("Global\\JoyconMapFile"));
	...
	JoyconMapBuffer = (LPVOID)MapViewOfFile(JoyconMapFile,   // handle to map object
	...
	if (JoyconMapBuffer == NULL)
	{
	_tprintf(TEXT("Could not map view of file (%d).\n"),GetLastError());

	CloseHandle(JoyconMapFile);

	}
	//*/

	//int y = CopyToBuffer(JoyconMapBuffer, TEXT("{'bDevice':0,'wAxisX':1,'wAxisXRot':2,'wAxisY':3,'wAxisYRot':4,'wAxisZ':5,'wAxisZRot':6,'wDial':7,'wSlider':8}"));
	...
	for (int _ = 0; _ < 100; ++_) {
		//linkoid
		for (int i = 0; i < 16; ++i) {
			trackers[i].tPolls.push_back(std::chrono::high_resolution_clock::now());
		}
	}
...
void exit() {
	...
	UnmapViewOfFile(JoyconMapBuffer);
	CloseHandle(JoyconMapFile);
...
void TestGLCanvas::OnPaint(wxPaintEvent& WXUNUSED(event)) {
	...
	canvas.DrawRotatedCube(trackers[0].quat);
...
void TestGLCanvas::OnKeyDown(wxKeyEvent& event) {
	...
	//linkoid - this seems like it would get in the way of things
