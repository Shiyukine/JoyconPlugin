    ...
    public class Joycon {
        ...
        private int ReceiveRaw() {
            ...
            if (ret > 0) {
                ...
                for (int n = 0; n < 3; n++) {
                    ...
                    JoyconPlugin.JoyconPlugin.onJoyconUpdate(this);
                }
            ...
            if (extraGyroFeature.Substring(0, 3) == "joy") {
                if (Config.Value("active_gyro") == "0" || active_gyro) {
                    ...
                    if (UseFilteredIMU) {
                        ...
                        dx = ((float)(GyroStickSensitivityX * (gyr_g.Z * dt))); // yaw
                        dy = (float)-(GyroStickSensitivityY * (gyr_g.Y * dt)); // pitch
                    }
        ...
        private void ExtractIMUValues(byte[] report_buf, int n = 0) {
            if (!isSnes) {
                ...
                /*if(true)
                {
                    acc_g.X = acc_r[0] * 0.000244f;
                    acc_g.Y = (!isLeft ? -1 : 1) * acc_r[1] * 0.000244f;
                    acc_g.Z = (!isLeft ? -1 : 1) * acc_r[2] * 0.000244f;

                    gyr_g.X = gyr_r[0] * 0.070f;
                    gyr_g.Y = (!isLeft ? -1 : 1) * gyr_r[1] * 0.070f;
                    gyr_g.Z = (!isLeft ? -1 : 1) * gyr_r[2] * 0.070f;
                }
                else */if (form.allowCalibration) {
                    for (int i = 0; i < 3; ++i) {
                        switch (i) {
                            case 0:
                ...
                AHRS.Update((float)(gyr_g.X * deg_to_rad), (float)(gyr_g.Y * deg_to_rad), (float)(gyr_g.Z * deg_to_rad), (float)acc_g.X, (float)acc_g.Y, (float)acc_g.Z);
            }
        }
